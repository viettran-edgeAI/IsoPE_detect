...
namespace mcu {
    ...
    // Provide a void_t implementation for older standards and use it for SFINAE-based detection.
    template<typename...>
    using void_t = void;
    ...
    template<typename U>
    struct less_comparable<U, void_t<decltype(std::declval<U>() < std::declval<U>())>>
