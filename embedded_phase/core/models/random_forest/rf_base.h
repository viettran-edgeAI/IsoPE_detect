...
namespace mcu {
    ...
    NOTE : Forest file components (with each model)
        ...
        2.  model_name_rf_config.json: model configuration file 
        3.  model_name_qtz.bin       : quantizer binary (feature quantizer, label mapping, outlier filtering flag)
        4.  model_name_forest.bin    : model file (all trees) in unified format
        5.  model_name_hogcfg.json   : HOG & Camera configuration file (for hog_transform - model preprocessing)
        6.  model_name_npd.bin       : node predictor file 
        7.  model_name_nlg.csv       : node splitting log file during training (for retraining node predictor)
        8.  model_name_ifl.bin       : inference log file (predictions, actual labels, metrics over time)
        9. model_name_tlog.csv       : time log file (detailed timing of forest events)
        10. model_name_mlog.csv      : memory log file (detailed memory usage of forest events)
    */

    using rf_label_type    = c_label_type;  // type for label related operations
    using sampleID_set     = ID_vector<sample_idx_type>;       // set of unique sample IDs
    using Rf_random        = eml_random;    // random number generator for RF

    static constexpr uint8_t         RF_MAX_LABEL_LENGTH    = EML_MAX_LABEL_LENGTH;     // max label length
    static constexpr uint8_t         RF_PATH_BUFFER         = EML_PATH_BUFFER;     // buffer for file_path(limit to 2 level of file)
    static constexpr uint8_t         RF_MAX_TREES           = 100;    // maximum number of trees in a forest
    static constexpr uint8_t         RF_MAX_LABELS          = EML_MAX_LABELS;    // maximum number of unique labels supported 
    ...
    static constexpr sample_idx_type  RF_MAX_SAMPLES         = 1048576;// maximum number of samples in a dataset (20 bits)
    ...
    private:
        void scan_current_resource() {
            ...
            build_file_path(filepath, "_rf_config.json");
            if (RF_FS_EXISTS(filepath)) {
    ...
    public:
        ...
        inline void get_config_path(char* buffer, int buffer_size = RF_PATH_BUFFER ) 
                        const { build_file_path(buffer, "_rf_config.json", buffer_size); }
            ...
            if (bn && strlen(bn) > 0) {
                ...
                auto rename_file = [&](const char* suffix) {
                    ...
                    if (RF_FS_EXISTS(old_file)) {
                        extern bool cloneFile(const char* src, const char* dest);
                ...
                rename_file("_rf_config.json");   // config file
