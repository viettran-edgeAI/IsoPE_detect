...
namespace eml {
	...
	class eml_quantizer_update_filter {
	private:
	...
	public:
		...
		size_t memoryUsage() const {
			return sizeof(*this) + mapping_.size() * sizeof(uint8_t);
		}
	};
	...
	// Template trait to check supported vector types for eml_quantizer
	template<typename T>
	...
	private:
		...
		label_type numLabels = (label_type)0;
		...
		static constexpr label_type ERROR_LABEL = EML_ERROR_LABEL<ProblemType>;
		...
		mutable vector<uint8_t> labelLengths;        // Cached label lengths (uint8_t sufficient for max 32-char labels)
		...
		bool storeLabel(label_type id, const char* label) {
			if (label == nullptr || (uint32_t)id >= (uint32_t)EML_MAX_LABELS) {
				return false;
			}

			if ((size_t)id >= labelOffsets.size()) {
				labelOffsets.resize((size_t)id + 1, UINT16_MAX);
			}
			if ((size_t)id >= labelLengths.size()) {
				labelLengths.resize((size_t)id + 1, 0);
			}

			// Update numLabels to reflect the highest label ID encountered
			if ((uint32_t)id >= (uint32_t)numLabels) {
				numLabels = (label_type)((uint32_t)id + 1);
			}
			...
			labelOffsets[(size_t)id] = offset;
			labelLengths[(size_t)id] = static_cast<uint8_t>(len);
		...
		__attribute__((always_inline)) inline int16_t quantizeValueSignal(uint16_t featureIdx, float value) const {
			...
			if (__builtin_expect(type == FT_DF, 1)) {
				...
				const int32_t clamped = (intValue < 0) ? 0 : ((intValue >= (int32_t)groupsPerFeature) ? ((int32_t)groupsPerFeature - 1) : intValue);
			...
			if (type == FT_CU) {
				...
				uint32_t scaledValue = static_cast<uint32_t>(adjusted <= (int64_t)std::numeric_limits<uint32_t>::max() ? adjusted : (int64_t)std::numeric_limits<uint32_t>::max());
	...
	public:
		...
		~eml_quantizer() {
			...
			featureMins.shrink_to_fit();
			featureMaxs.shrink_to_fit();
			featureBaselinesScaled.shrink_to_fit();
			featureScales.shrink_to_fit();
			allEdgesScaled.shrink_to_fit();
			edgeOffsets.shrink_to_fit();
			edgeCounts.shrink_to_fit();
			allDiscreteValuesF.shrink_to_fit();
			dcOffsets.shrink_to_fit();
			dcCounts.shrink_to_fit();
			labelOffsets.shrink_to_fit();
			labelLengths.shrink_to_fit();
			labelStorage.shrink_to_fit();

		#ifndef EML_STATIC_MODEL
			featureMeans.clear();
			featureStdDevs.clear();
			featureMeans.shrink_to_fit();
			featureStdDevs.shrink_to_fit();
		#endif
		}
		...
		void init(uint16_t num_features, uint8_t num_labels) {
			isLoaded = false;
			numFeatures = num_features;
			numLabels = (label_type)0; // Will be set by storeLabel or load
		...
		bool loadQuantizer(const char* file_path) {
			if (isLoaded) {
				return true;
			}

			if (file_path == nullptr) {
				eml_debug(0, "❌ Load Quantizer failed: Path is null");
				return false;
			}
			...
			labelOffsets.assign(labelCount, UINT16_MAX);
			labelLengths.assign(labelCount, 0);
			...
			for (uint8_t i = 0; i < labelCount; ++i) {
				...
				if (labelLen > 0) {
					...
					if (!storeLabel((label_type)labelId, labelBuffer)) {
			...
			eml_debug_2(2, "   Labels: ", (uint32_t)numLabels, ", Outlier filtering: ", removeOutliers ? "enabled" : "disabled");
			return true;
		}
		...
		bool apply_concept_drift_update(const vector<eml_drift_sample>& drift_samples, eml_quantizer_update_filter& out_filter) {
			return apply_concept_drift_update_impl(drift_samples, out_filter);
		}

		template<typename DriftSample>
		bool apply_concept_drift_update(const vector<DriftSample>& drift_samples, eml_quantizer_update_filter& out_filter) {
			return apply_concept_drift_update_impl(drift_samples, out_filter);
		}

	private:
		template<typename DriftSample>
		bool apply_concept_drift_update_impl(const vector<DriftSample>& drift_samples, eml_quantizer_update_filter& out_filter) {
			...
			const uint16_t bins = groupsPerFeature;
	...
	public:
		...
		bool apply_fifo_bin_shrink(Dataset& loaded_train_data, eml_quantizer_update_filter& out_filter, uint8_t maxBinsToShrink = 2) {
			if (!isLoaded || numFeatures == 0 || groupsPerFeature == 0) {
				return false;
			}
			...
			eml_quantizer_update_filter tempFilter;
			...
			for (uint16_t f = 0; f < numFeatures; ++f) {
				...
				const sample_idx_type n = loaded_train_data.size();
				for (sample_idx_type si = 0; si < n; ++si) {
		...
		size_t memory_usage() const {
			...
			usage += sizeof(numFeatures) + sizeof(groupsPerFeature) + sizeof(numLabels) + sizeof(quantization_coefficient) + sizeof(isLoaded);
			usage += 4;
		...
		void enable_outliner_zscore_filtering() { removeOutliers = true; }
		void disable_outliner_zscore_filtering() { removeOutliers = false; }
	#endif

		const char* getOriginalLabelPtr(label_type normalizedLabel) const {
			if ((size_t)normalizedLabel >= labelOffsets.size()) {
				return nullptr;
			}
			uint16_t offset = labelOffsets[(size_t)normalizedLabel];
		...
		bool getOriginalLabelView(label_type normalizedLabel, const char** outData, uint16_t* outLength = nullptr) const {
			if (!outData || (uint32_t)normalizedLabel >= (uint32_t)EML_MAX_LABELS) {
				return false;
			}
			...
			*outData = data;
			if (outLength) {
				uint8_t cached = labelLengths[(size_t)normalizedLabel];
				if (cached == 0) {
					cached = static_cast<uint8_t>(strlen(data));
					if (labelLengths[(size_t)normalizedLabel] == 0) {
						labelLengths[(size_t)normalizedLabel] = cached;
					}
				}
				*outLength = cached;
			}
		...
		bool getOriginalLabel(label_type normalizedLabel, char* buffer, size_t bufferSize) const {
			if (!buffer || bufferSize == 0 || (uint32_t)normalizedLabel >= (uint32_t)EML_MAX_LABELS) {
				return false;
			}
			...
			uint8_t length = labelLengths[(size_t)normalizedLabel];
			if (length == 0) {
				buffer[0] = '\0';
				return true;
			}
		...
		label_type getNormalizedLabel(const char* originalLabel) const {
			...
			for (size_t i = 0; i < labelOffsets.size(); ++i) {
		...
		label_type addNewLabel(const char* label) {
			...
			if (existing != ERROR_LABEL) {
				return existing;
			}

			if ((uint32_t)numLabels >= (uint32_t)EML_MAX_LABELS) {
