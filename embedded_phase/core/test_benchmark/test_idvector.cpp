...
public:
    ...
    // Test clear() and shrink_to_fit() combination (previously caused core dump)
    void test_clear_fit_safety() {
        ...
        // Test 1: Basic clear() and shrink_to_fit() on populated vector
        {
            ...
            vec.shrink_to_fit();
            test_passed = vec.empty();
            assert_critical_test(test_passed, "Vector remains empty after shrink_to_fit() on cleared vector [CORE DUMP FIX]");
            if (!test_passed) all_clear_fit_tests_passed = false;
            
            test_passed = (vec.size() == 0);
            assert_critical_test(test_passed, "Size remains 0 after shrink_to_fit() on cleared vector [CORE DUMP FIX]");
            if (!test_passed) all_clear_fit_tests_passed = false;
        }
        
        // Test 2: Multiple clear() and shrink_to_fit() calls
        {
            ...
            vec.clear();
            vec.shrink_to_fit(); // First shrink after clear
            vec.shrink_to_fit(); // Second shrink - should be safe
            assert_test(vec.empty(), "Vector remains empty after multiple fits");
        }
        
        // Test 3: clear(), add elements, then shrink_to_fit()
        {
        ...
        // Test 4: Test with different template parameters
        {
            ...
            small_vec.clear();
            small_vec.shrink_to_fit(); // Should not crash
            ...
            large_vec.clear();
            large_vec.shrink_to_fit(); // Should not crash
            assert_test(large_vec.empty(), "uint32_t BPV=3 vector safe after clear+fit");
        }
        
        // Test 5: Edge case - shrink_to_fit() on empty vector from construction
        {
            ID_vector<uint16_t, 1> vec; // Default construction
            assert_test(vec.empty(), "Vector empty from construction");
            
            vec.shrink_to_fit(); // Should be safe even on never-populated vector
